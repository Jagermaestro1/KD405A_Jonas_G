...
public class HouseGUI extends JFrame {
	...
	public HouseGUI() {
		...
		for (int qq = 0; qq < 100; qq++) {
			int ra = r.nextInt(216) + 1800; // Skillnaden mellan årtalen +
											// startår
			int sa = s.nextInt(991) + 10; // Skillnaden mellan storlekarna + min
											// storlek
		...
		for (int q = 0; q < houses.length; q++) {
			if (houses[q].getYearBuilt() != 0 && houses[q].getSize() != 0) {
				textArea.append("Hus " + q + " är byggt år:" + houses[q].getYearBuilt() + " och är "
						+ houses[q].getSize() + "kvm stort. Grattis!\n");
			...
			for (House h : bostadsarea) {
				if (h.getYearBuilt() != 0 && h.getSize() != 0) {
					textArea.append("Ett hus är byggt år:" + h.getYearBuilt() + " och är " + h.getSize()
							+ "kvm stort. Grattis!\n");
